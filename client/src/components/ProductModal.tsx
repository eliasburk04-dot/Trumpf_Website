...
const formatSpecValue = (key: string, value: unknown) => {
  if (typeof value === "object" && value !== null) {
    return (
      <div className="flex flex-col gap-1 mt-1 bg-slate-50 dark:bg-slate-800 p-2 rounded-md border border-slate-100 dark:border-slate-700">
        {Object.entries(value).map(([k, v]) => (
          <div
            key={k}
            className="flex justify-between items-center text-[13px] border-b border-slate-200/60 dark:border-slate-700 pb-1 last:border-0 last:pb-0"
          >
            <span className="text-muted-foreground font-normal">{k}</span>
            <span className="font-semibold text-slate-700 dark:text-slate-200">{String(v)} mm</span>
          </div>
        ))}
      </div>
    );
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto" data-testid="modal-product">
        ...
        <div className="grid md:grid-cols-2 gap-8 mt-6">
          <div className="space-y-6">
            ...
            {product.highlights && (
              <div className="bg-slate-50 dark:bg-slate-800 rounded-xl p-5">
                <h4 className="font-bold text-slate-800 dark:text-white mb-4">Highlights</h4>
          ...
          <div className="space-y-6">
            <div>
              <div className="inline-block bg-primary/10 text-primary font-bold px-4 py-2 rounded-full text-lg mb-4">
                {product.price}
              </div>
              ...
              <div className="flex flex-wrap gap-3">
                <Button className="w-full shadow-lg hover:shadow-xl transition-shadow" size="lg" data-testid="modal-btn-contact" asChild>
                  <a href="tel:+497141921912">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {Object.entries(product.specs).map(([key, value]) => (
                      <div key={key} className="space-y-1">
                        ...
                        <div className="font-medium text-sm">{formatSpecValue(key, value)}</div>
                      </div>
                    ))}
                  </div>
                ) : (
